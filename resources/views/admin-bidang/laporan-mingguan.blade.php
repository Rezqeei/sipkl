<x-admin-bidang-layout>
    <div class="p-6 md:p-10" x-data="{ showModal: false, selectedLaporan: null }">
        <h1 class="text-3xl font-bold text-gray-800 mb-6">Verifikasi Laporan Mingguan</h1>

        <div class="bg-white p-8 rounded-xl shadow-lg border">
            <h2 class="text-xl font-semibold mb-4">Daftar Laporan Masuk</h2>
            <div class="overflow-x-auto">
                <table class="min-w-full bg-white">
                    <thead class="bg-gray-100">
                        <tr>
                            <th class="py-3 px-6 text-left">Nama Mahasiswa</th>
                            <th class="py-3 px-6 text-center">Minggu Ke</th>
                    ...
                    <tbody>
                            ...
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="py-4 px-6 text-center">
                                    <div class="flex justify-center gap-2">
                                        <form method="POST" action="{{ route('admin-bidang.laporan-mingguan.update', $laporan) }}">
                                            @csrf
                        ...
                        @empty
                            <tr><td colspan="4" class="py-4 px-6 text-center text-gray-500">Tidak ada laporan baru yang perlu diverifikasi.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div x-show="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" x-cloak>
            <div @click.away="showModal = false" class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
                ...
                <form :action="`/admin-bidang/laporan-mingguan/${selectedLaporan ? selectedLaporan.id_laporan_mingguan : ''}`" method="POST">
                    @csrf
