    ...
    <div class="p-6 md:p-10" x-data="{ showModal: false, selectedDokumen: null, selectedAntrian: null }">
                            ...
                            <td class="py-4 px-6">
                                ...
                                <div class="flex flex-col space-y-1">
                                    ...
                                    <a href="{{ route('admin-instansi.verifikasi-dokumen.download', ['dokumen' => $antrian->dokumen, 'tipe' => 'bankesbangpol']) }}"
                                        class="text-blue-600 hover:underline text-sm">Lihat Surat Bankesbangpol</a>
                                </div>
                                @else
        ...
        <div x-show="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
            ...
            <div @click.away="showModal = false" class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
                ...
                <form
                    :action="`/admin-instansi/verifikasi-dokumen/${selectedDokumen ? selectedDokumen.id_dokumen : ''}`"
                    method="POST">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea name="catatan_revisi" rows="4" class="w-full p-2 border rounded-lg"
                            required></textarea>
                    </div>
                    <div class="mt-6 flex justify-end gap-4">
                        <button type="button" @click="showModal = false"
                            class="bg-gray-200 py-2 px-4 rounded-lg">Batal</button>
                        <button type="submit" class="bg-yellow-500 text-white py-2 px-4 rounded-lg">Kirim
                            Revisi</button>
                    </div>
                </form>
            </div>
