                    ...
                    <div class="mb-6 p-4 border rounded-md">
                        ...
                        <form action="{{ route('admin-instansi.manajemen-bidang.store') }}" method="POST">
                            ...
                            <div>
                                ...
                                <x-text-input id="nama_bidang" class="block mt-1 w-full" type="text" name="nama_bidang"
                                    :value="old('nama_bidang')" required autofocus />
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200"
                                x-data="{ openEdit: false, openDelete: false, selectedBidang: {}, deleteUrl: '' }">
                                @forelse ($bidangs as $index => $bidang)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $index + 1 }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{
                                        $bidang->nama_bidang }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button @click="openEdit = true; selectedBidang = {{ $bidang }}"
                                            class="text-indigo-600 hover:text-indigo-900">Edit</button>
                                        <button
                                            @click="openDelete = true; deleteUrl = '{{ route('admin-instansi.manajemen-bidang.destroy', $bidang->id) }}'"
                                ...
                                <template x-if="openEdit">
                                    <div class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center"
                                        ...
                                        <div class="bg-white p-6 rounded-lg shadow-xl max-w-md w-full"
                                            ...
                                            <form
                                                :action="'{{ url('admin-instansi/manajemen-bidang') }}/' + selectedBidang.id"
                                                method="POST">
                                                @csrf
                                                @method('PUT')
                                                <div>
                                                    <x-input-label for="edit_nama_bidang" :value="__('Nama Bidang')" />
                                                    <x-text-input id="edit_nama_bidang" class="block mt-1 w-full"
                                                        type="text" name="nama_bidang"
                                                        x-model="selectedBidang.nama_bidang" required />
                                                </div>
                                                <div class="mt-6 flex justify-end">
                                                    <x-secondary-button @click="openEdit = false">Batal
                                                    </x-secondary-button>
                                ...
                                <template x-if="openDelete">
                                    <div class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center"
                                        ...
                                        <div class="bg-white p-6 rounded-lg shadow-xl max-w-md w-full"
                                            ...
                                            <p class="mt-1 text-sm text-gray-600">Apakah Anda yakin ingin menghapus
                                                bidang ini? Tindakan ini tidak dapat dibatalkan.</p>
                                            <form :action="deleteUrl" method="POST" class="mt-6 flex justify-end">
                                                @csrf
                                                @method('DELETE')
                                                <x-secondary-button @click="openDelete = false">Batal
                                                </x-secondary-button>
